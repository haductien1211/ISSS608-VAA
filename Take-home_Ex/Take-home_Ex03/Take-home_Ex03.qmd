...
fig1 <- BO_owners_count %>%
  ...
  plot_ly(x = ~start_date,
          ...
          mode = 'lines+markers',
          text = ~paste("Day: ", start_date, 
                        "<br>Own: ", BeneficialOwnership_total),
          hoverinfo = 'text',
          fill = 'tozeroy',
          transforms = list(
            list(
...
```{r}
mc3_edges_final <- mc3_edges %>%
  filter(type == "Event.Owns.Shareholdership"|
         type == "Event.Owns.BeneficialOwnership")
View(mc3_edges_final)
```

```{r}
mc3_edges_WorksFor_count <- mc3_edges %>%
  select(target, nodes_type_target, type) %>%
  group_by(target, type) %>%
  mutate(WorksFor_count = n()) %>%
  distinct() %>%
  pivot_wider(names_from = type, values_from = WorksFor_count) %>%
  select(target,nodes_type_target, Event.WorksFor) %>%
  filter(str_like(nodes_type_target, "%Entity.Organization%"))
