...
```{r}
ggplot(data = first_quarter_2024, aes(x = `Sale Date FORMAT`, y = `Unit Price ($ PSM)`)) +
  # scale_x_discrete(name ="Sale Month", 
  #                limits = c("Jan 2023", "Feb 2023", "Mar 2023", 
  #                   "Apr 2023","May 2023","Jun 2023", 
  #                   "Jul 2023", "Aug 2023", "Sep 2023",
...
```{r}
ggplot(data = first_quarter_2024, aes(x = `Sale Date asDate`, y = `Unit Price ($ PSM)`)) +
  geom_path(aes(color = `Property Type`), stat="summary", fun ="median", size=0.6) +
  theme(axis.text=element_text(size=6)) +
  ggtitle("Daily changes in mean Unit Price ($PSM) for first quarter 2024")
```

```{r}
ggplot(data = first_quarter_2024, aes(y = `Area (SQM)`, x = `Transacted Price ($)`)) +
  geom_point()+
  geom_smooth(method = glm, linewidth = 1) +
  coord_cartesian(ylim = c(30,450), xlim=c(450000,8000000)) +
  geom_label_repel(aes(label = `Postal Code`), fontface = "bold", max.overlaps = 6) +
