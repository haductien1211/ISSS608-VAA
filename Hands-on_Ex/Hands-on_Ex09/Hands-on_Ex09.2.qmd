...
In this hands-on exercise, we will learn how to plot data visualisation for visualising correlation matrix with R. It consists of three main sections. First, we will learn how to create correlation matrix using `pairs()` of [R Graphics](https://www.rdocumentation.org/packages/graphics/versions/3.6.0/topics/pairs). Next, we will learn how to plot [corrgram](https://www.datavis.ca/papers/corrgram.pdf) using **corrplot** package of R. Lastly, we will learn how to create an interactive correlation matrix using plotly R.
...
Let us import the data into R by using `read_csv()` of **readr** package
...
Since ggstasplot is an extension of ggplot2, it also supports faceting. However the feature is not available in `ggcorrmat()` but in the `grouped_ggcorrmat()` of [ggstatsplot](https://indrajeetpatil.github.io/ggstatsplot/reference/grouped_ggcorrmat.html).
...
grouped_ggcorrmat(
  ...
  annotation.args = list(
    tag_levels = "a",
    title = "Correlogram for wine dataset",
    subtitle = "The measures are: alcohol, sulphates, fixed acidity, 
    citric acid, chlorides, residual sugar, density, 
    free sulfur dioxide and volatile acidity",
...
-   `plotgrid.args` argument provides a list of additional arguments passed to [patchwork::wrap_plots](https://patchwork.data-imaginist.com/reference/wrap_plots.html), except for guides argument which is already separately specified earlier.
